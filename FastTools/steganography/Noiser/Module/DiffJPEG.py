# Code ported from https://github.com/jzyustc/MBRS/blob/main/network/noise_layers/jpeg.py
# *************************************************************************
...
class JpegBasic(nn.Module):
    ...
    def std_quantization(self, image_yuv_dct, scale_factor, round_func=torch.round):
        ...
        q_image_yuv_dct[:, :1, :, :] = image_yuv_dct[:,
                                                     :1, :, :] / luminance_quant_tbl
        q_image_yuv_dct[:, 1:, :, :] = image_yuv_dct[:,
                                                     1:, :, :] / chrominance_quant_tbl
    ...
    def std_reverse_quantization(self, q_image_yuv_dct, scale_factor):
        ...
        image_yuv_dct[:, :1, :, :] = q_image_yuv_dct[:,
                                                     :1, :, :] * luminance_quant_tbl
        image_yuv_dct[:, 1:, :, :] = q_image_yuv_dct[:,
                                                     1:, :, :] * chrominance_quant_tbl
    ...
    def dct(self, image):
        ...
        image_dct = torch.cat(torch.cat(image_dct.chunk(
            split_num2, 0), 3).chunk(split_num1, 0), 2)
        return image_dct
    ...
    def idct(self, image_dct):
        ...
        image = torch.cat(torch.cat(image.chunk(
            split_num2, 0), 3).chunk(split_num1, 0), 2)
    ...
    def rgb2yuv(self, image_rgb):
        ...
        image_yuv[:, 2:3, :, :] = 0.5 * image_rgb[:, 0:1, :, :] \
            - 0.4187 * image_rgb[:, 1:2, :, :] - \
            0.0813 * image_rgb[:, 2:3, :, :]
        return image_yuv

    def yuv2rgb(self, image_yuv):
        image_rgb = torch.empty_like(image_yuv)
        image_rgb[:, 0:1, :, :] = image_yuv[:, 0:1, :, :] + \
            1.40198758 * image_yuv[:, 2:3, :, :]
        image_rgb[:, 1:2, :, :] = image_yuv[:, 0:1, :, :] - 0.344113281 * image_yuv[:, 1:2, :, :] \
                                                          - 0.714103821 * \
            image_yuv[:, 2:3, :, :]
        image_rgb[:, 2:3, :, :] = image_yuv[:, 0:1, :, :] + \
            1.77197812 * image_yuv[:, 1:2, :, :]
        return image_rgb
    ...
    def subsampling(self, image, subsample):
        if subsample == 2:
            ...
            image_block = torch.cat(
                torch.cat(image.split(8, 2), 0).split(8, 3), 0)
            for i in range(8):
                if i % 2 == 1:
            ...
            image = torch.cat(torch.cat(image_block.chunk(
                split_num2, 0), 3).chunk(split_num1, 0), 2)
        return image

@NoiseLayerManager.register("diff_jpeg")
class DiffJpeg(JpegBasic):
    """
    ...
    def forward(self, image):
        ...
        # quantization
        image_quantization = self.std_quantization(
            image_dct, self.scale_factor)

        # reverse quantization
        image_quantization = self.std_reverse_quantization(
            image_quantization, self.scale_factor)
        ...
        if self.padding:
            noised_image = noised_image[:, :,
                                        h:noised_image.shape[2]-h, w:noised_image.shape[3]-w]
        noised_image = noised_image.clamp(-1, 1)
        if channel == 1:
            noised_image = torch.mean(noised_image, dim=1, keepdim=True)
        noised_image = (noised_image + 1) / 2
        noised_image = noised_image * (maxx - minn) + minn
