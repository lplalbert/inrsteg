...
class JND(nn.Module):
    ...
    def forward(self, cover_img, watermark, factor=1.0):
        """
            cover_img in [0, 1]
            watermark in [0, 1]
            factor in [0, 1] 代表修改强度，1是完全按照JND模型修改，0是不修改
        """
        heatmap = self.heatmaps(cover_img)
        watermark = factor * heatmap * watermark + (1 - factor) * watermark
        return watermark
        pass
...
if __name__ == "__main__":
    model = JND()
    cover_img = torch.rand(1, 3, 256, 256)
    watermark = torch.rand(1, 3, 256, 256)
    out = model(cover_img, watermark)
    pass