...
class VQEmbed(nn.Module):
    """VQ化层，来自VQVAE"""
    def __init__(self, num_emb, emb_dim, commitment_factor: int = 0.25):
        """初始化

        Args:
            num_emb (_type_): vq层的个数
            emb_dim (_type_): emb的维度
            commitment_factor : commit的权重因子，一般默认就好
        """
        super(VQEmbed, self).__init__()
        self.num_emb = num_emb
        self.emb_dim = emb_dim
        self.commit_factor = commitment_factor
        self.emb = NearestEmbed(num_emb, emb_dim)
        pass
    ...
    def forward(self, z_e):
        ...
        Returns:
            ...
            loss : VQ层的损失
        """
        z_q, _ = self.emb(z_e, True)
        emb, _ = self.emb(z_e.detach())
        loss = F.mse_loss(emb, z_e.detach()) + self.commit_factor * F.mse_loss(z_e, emb.detach())
        return z_q, loss
        pass
    pass
