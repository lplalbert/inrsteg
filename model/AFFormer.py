...
class MultiHaarLayer(nn.Module):
    def __init__(self, in_channel, level=1, drop_hp=False):
        super().__init__()
        self.in_channel = in_channel
        if not drop_hp:
            self.haar_layer = nn.ModuleList([HaarTransform(in_channel * (4 ** k)) for k in range(level)])
        else:
            self.haar_layer = nn.ModuleList([HaarTransform(in_channel) for k in range(level)])
        self.drop_hp = drop_hp
        pass
    
    def forward(self, x):
        for layer in self.haar_layer:
            x = layer(x, rev=False)
            if self.drop_hp:
                x = x[:,:self.in_channel, :, :]
        return x
...
class afformer_base(AFFormer):
    def __init__(self, embed_dims=[32, 96, 176, 216], **kwargs):
...
class AFFormerDecoder(nn.Module):
    def __init__(self, num_bit, redundancy=3):
        super().__init__()
        self.aff_dim = 96+176+216+216
        self.num_bit = num_bit
        self.redundancy = redundancy
        self.model = afformer_base()
        self.predictor = MLP(self.aff_dim, num_bit*redundancy, 256, norm='bn', act='lrelu', num_hidden_layers=2)
        pass
    ...
    def forward(self, x):
        ...
        for feat in feats:
            ...
            feat = feat.flatten(1)
            msg_feat.append(feat)
            # print(feat.size())
        msg_feat = torch.cat(msg_feat, dim=-1)
        msg = self.predictor(msg_feat)
        msg = msg.view(-1, self.num_bit, self.redundancy)
...
if __name__ == "__main__":
    x = torch.randn(10, 3, 124, 654)
    model = AFFormerDecoder(30)
    print(model(x).size())
    pass