...
class INRMark(EngineModel):
    ...
    def __init__(self, args: Args):
        ...
        self.inr = nn.ModuleList([
            LinearBlock(self.struct_dim + self.msg_dim, self.struct_dim, 'bn', 'relu'),
            LinearBlock(self.struct_dim, self.struct_dim, 'bn', 'relu'),
            LinearBlock(self.struct_dim, self.struct_dim, 'bn', 'relu'),
            LinearBlock(self.struct_dim, self.struct_dim, 'bn', 'relu'),
        ])
    ...
    def render_img(self,
        ...
        struct_feat = torch.cat([struct_feat, msg_feat], dim=-1)
        # Inference
        for layer in self.inr:
            struct_feat = layer(struct_feat)
            pass
