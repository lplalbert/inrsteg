...
class INRMark(EngineModel):
    def __init__(self, args):
        ...
        self.inr = ResMLP(self.feat_channel * 2 + self.mapper_dim + self.diff_coords_dim + 3, 3, num_layers=6, hidden_dim=256, norm='none', activation='relu')
    ...
    def render_img(self, coords, msg, img):
        ...
        if self.inject_rgb:
            feat = torch.cat([struct_feat, diff_coords_feat, coords_feat, msg_feat, rgb], dim=-1)
        else:
            zeros = torch.zeros_like(rgb)
            feat = torch.cat([struct_feat, diff_coords_feat, coords_feat, msg_feat, zeros], dim=-1)
