...
class LPIPS(nn.Module):
    def __init__(self, no_negative=False):
    ...
    def forward(self, real_x, fake_x):
        ...
        if self.no_negative:
            return sum([spatial_average(torch.abs(self.lins[i].model(diffs[i]))) for i in range(len(self.channels))])
...
class REC_LPIPS(nn.Module):
    def __init__(self, lpips_factor=1, rec_factor=1):
        super(REC_LPIPS, self).__init__()
    ...
    def forward(self, fake_x, real_x):
        # 输入[-1, 1]
        self.eval()
        perceptual_loss = self.lpips(real_x, fake_x) * self.lpips_factor
        rec_loss = torch.abs(real_x - fake_x) * self.rec_factor
        lpips_rec_loss = perceptual_loss + rec_loss
        return lpips_rec_loss.mean()
        pass
    pass
if __name__ == "__main__":
    x = torch.clamp(torch.randn(2, 3, 128, 128), -1, 1)
    y = x + 0.1 * torch.clamp(torch.randn(2, 3, 128, 128), -1, 1)
    model = REC_LPIPS()
    print(model(x, y))
