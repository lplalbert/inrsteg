...
# Local imports
import FastTools.steganography.Noiser.Module.WeChatHF       # noqa: F401  register WeChatHFZero
import FastTools.steganography.Noiser.Module.ScreenShooting  # noqa: F401  register ScreenShooting
...
class FeatureGrid(nn.Module):
    ...
    def __init__(self,
        ...
        # Create multi-resolution grids (v8: reduced init scale for smoother convergence)
        init_scale = 0.01
        self.grids = nn.ParameterList([
            nn.Parameter(torch.randn(
                1, feat_dim, img_size // 2**i, img_size // 2**i) * init_scale)
        ...
        if addition_fourier_feat:
            self.adapter = nn.Sequential(
                nn.Linear(feat_dim * level + fourier_feat_dim, feat_dim * level),
                nn.BatchNorm1d(feat_dim * level),
                nn.SiLU()
            )
        else:
...
class INRMark(EngineModel):
    ...
    def __init__(self, args: Args):
        ...
        self.level_dim = 32
        self.level_num = 4  # v8: 减少网格层级，抑制大块条纹
        self.msg_dim = 128
        self.rank_dim = 64  # v8: 加大融合秩，解耦消息长度瓶颈
        self.alpha = args.alpha
        # Loss weights
        self.w_msg = args.w_msg
        self.w_img = args.w_img
        self.w_lpips = args.w_lpips
        self.w_tv = getattr(args, 'w_tv', 0.01)
        self.struct_dim = 256
        self.decoder_mask_len = getattr(args, 'decoder_mask_len', args.msg_len)
    ...
    def _load_pretrained(self, ckpt_path: str):
        ...
        # Key remapping: pretrained v6 uses 'norm' for BN in LowRankFusionBlock, v8 uses 'bn'
        remapped = {}
        ...
        print(f"[v8] Pretrained loaded: {matched} matched, {skipped} skipped from {ckpt_path}")
    ...
    def render_img(self,
        ...
        if img is not None:
            ...
            return residual * self.alpha, residual
    ...
    def tv_loss(self):
        """Grid Total Variation: penalize large differences between adjacent grid points."""
        tv = 0.0
        for grid in self.struct_embedding.grids:
            # grid: [1, C, H, W]
            dh = (grid[:, :, 1:, :] - grid[:, :, :-1, :]).abs().mean()
            dw = (grid[:, :, :, 1:] - grid[:, :, :, :-1]).abs().mean()
            tv += dh + dw
        return tv / len(self.struct_embedding.grids)
    ...
    def custom_train_step(self, batch, optimizers, schedulers, batch_idx):
        ...
        tv = self.tv_loss() * self.w_tv
        loss = msg_loss + img_loss + tv
        ...
        self.log('metric/tv_loss', tv.cpu().item())
    ...
    def custom_valid_step(self, batch, batch_idx):
        ...
        # 采用mask，只看前n位
        predict_msg = predict_msg[:, :self.decoder_mask_len]
        msg = msg[:, :self.decoder_mask_len]
    ...
    def build_optimizers(self, args):
        # v8: differential LR — struct_embedding learns slowly (stable embedding),
        # decoder learns fast (adapts to current embedding)
        lr = 4e-4
        struct_params = list(self.struct_embedding.parameters())
        decoder_params = list(self.decoder.parameters())
        other_params = [p for n, p in self.named_parameters()
                        if not n.startswith('struct_embedding') and not n.startswith('decoder')]
        param_groups = [
            {'params': struct_params, 'lr': lr * 0.3, 'name': 'struct_embedding'},
            {'params': decoder_params, 'lr': lr * 3.0, 'name': 'decoder'},
            {'params': other_params, 'lr': lr, 'name': 'other'},
        ]
        return torch.optim.RAdam(param_groups, lr=lr)
        pass
