...
from model.affblock.layer import ConvLayer, InvertedResidual
...
class AFFBlock(BaseModule):
    ...
    def __init__(
        ...
        self.enable_coreml_compatible_fn = False or False
        ...
        global_rep.append(nn.BatchNorm2d(num_features=transformer_dim))
...
if __name__ == "__main__":
    ...
    print(input.shape)
    affblock = AFFBlock(in_channels=input.shape[1], ffn_dim=256, head_dim=head_dim)
    # torch.save(affblock.state_dict(), "AFFBlock.ckpt")

    output = affblock(input)
    print(output.shape)