...
# 感觉 diff coords是没必要的，直接将x,y用siren做变换，然后使用线性插值的内在tensor拼接就好了吧，我觉得，没必要整那么复杂
# 直接MLP不行的原因是，没用bn，现在有了bn感觉已经稳定收敛了
# 视觉损失和解码损失仍然是个问题，这个平衡问题有点难以解决
...
class INRMark(EngineModel):
    def __init__(self, args):
        ...
        # self.lpips = LPIPS(net='alex', variant="shift_tolerant")
        ...
        #     # ImageNormalizer(),
        #     decoder,
        #     MLP(48, self.msg_len, hidden_dim=128, num_hidden_layers=1, norm='bn', act='relu'),
        # )
        ...
        # self.inr = ResMLP(self.level_dim * self.level_num + self.diff_coords_dim + self.feat_channel + 3 + cell_dim, 3, num_layers=6, hidden_dim=256, norm='bn', activation='relu')
        self.inr = MLP(self.level_dim * self.level_num + self.diff_coords_dim + self.feat_channel + 3 + cell_dim, 3, num_hidden_layers=8, hidden_dim=256, norm='bn', act='relu')
    ...
    def render_img(self, coords, msg, img, cell=None):
        ...
        if self.use_cell:
            feat = torch.cat([struct_feat, diff_coords_feat, msg_feat, rgb, cell_feat], dim=-1)
        else:
