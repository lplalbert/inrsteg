...
from FastTools.module.common import MLP, Conv2dBlock, ConvBNRelu, LinearBlock, ResBlock
...
class FeatureGrid(nn.Module):
    def __init__(self, img_size, feat_dim=64, level=4):
        super().__init__()
    ...
    def forward(self, coords):
        ...
        for feat_grid in self.grids:
            ...
            grid = feat_grid.expand(coords.shape[0], -1, -1, -1)
            feat = F.grid_sample(grid, coords.flip(-1), align_corners=True)
...
class INRMark(EngineModel):
    def __init__(self, args):
        ...
        self.struct_embedding = FeatureGrid(self.img_size * 2, self.feat_channel // 4, level=4)
        self.pe = FourierFeatMapping(2, 32, self.mapper_dim)
        
        
        self.msg_encoder = nn.Sequential(MLP(self.msg_len, self.feat_channel, num_hidden_layers=4, norm='bn'))
        ...
        self.decoder = nn.Sequential(
            # DecoderNorm(),
            decoder,
            MLP(48, self.msg_len, hidden_dim=256, num_hidden_layers=1, norm='bn', act='lrelu'),
        )
        
        self.inr = ResMLP(self.feat_channel + self.mapper_dim + 3, 3, num_layers=6, hidden_dim=256, norm='none', activation='relu', final_bias=False)
        # self.inr = FCBlock(self.feat_channel * 2 + self.mapper_dim + 3, 3, 6, 256, False, 'relu')
    ...
    def render_img(self, coords, msg, img):
        ...
        coords_feat = rearrange(coords, 'b h w c -> b (h w) c')
        coords_feat = self.pe(coords_feat)
        msg_feat = self.msg_encoder(msg) # [bs, feat_channel]
        msg_feat = msg_feat.unsqueeze(1).expand(-1, coords_feat.size(1), -1)
        rgb = rearrange(img, 'b c h w -> b (h w) c')
        
        struct_feat = struct_feat * msg_feat
        if self.inject_rgb:
            feat = torch.cat([struct_feat, coords_feat, rgb], dim=-1)
        else:
            zeros = torch.zeros_like(rgb)
            feat = torch.cat([struct_feat, coords_feat, zeros], dim=-1)
            
        mask = self.inr(feat)
        # mask = F.tanh(mask)
        wm_img = torch.clamp(mask + rgb, 0, 1)
