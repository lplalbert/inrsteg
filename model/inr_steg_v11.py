...
class INRMark(EngineModel):
    def __init__(self, args):
        ...
        # self.feature_grid = FeatureGrid(self.img_size * 2, self.feat_channel // 4, )
        ...
        self.inr = ResMLP(self.feat_channel * 2 + self.mapper_dim + 3, 3, num_layers=6, hidden_dim=256, norm='none', activation='relu')
    ...
    def render_img(self, coords, msg, img):
        ...
        if self.inject_rgb:
            feat = torch.cat([struct_feat, coords_feat, msg_feat, rgb], dim=-1)
        else:
            zeros = torch.zeros_like(rgb)
            feat = torch.cat([struct_feat, coords_feat, msg_feat, zeros], dim=-1)
        mask = self.inr(feat)
        wm_img = torch.clamp(mask + rgb, 0, 1)
        wm_img = rearrange(mask, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        if self.fixed_psnr:
            wm_img = clip_psnr(wm_img, img, psnr=self.fixed_psnr, over_clip=True)
        return wm_img
        pass
