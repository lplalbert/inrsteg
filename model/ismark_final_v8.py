...
class INRMark(EngineModel):
    def __init__(self, args):
        ...
        self.msg_encoder = MLP(self.msg_len, self.feat_channel, num_hidden_layers=4, norm='bn', hidden_dim=256)
            

        # self.decoder = AFFormerDecoder(self.msg_len, 1)
        self.decoder = get_hidden_decoder(self.msg_len, 3, 8)
        self.inr = MLP(self.level_dim * self.level_num + self.feat_channel*2, 3, num_hidden_layers=8, hidden_dim=256, norm='bn', act='relu')
    ...
    def render_img(self, coords, msg, img, cell=None):
        ...
        msg_feat = msg_feat.unsqueeze(1).expand(-1, n, -1) # [bs, n, dim]
        msg_feat = rearrange(msg_feat, "b n c -> (b n) c")
        
        # 含有水印信息的结构特征
        # wm_struct_feat = struct_feat * msg_feat
        
        # 解码mask
        feat = torch.cat([struct_feat, msg_feat, coords_feat], dim=-1)
        mask = self.inr(feat)
